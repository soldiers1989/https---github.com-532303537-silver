	...
	public Table findAllCategory() {
		...
		try {
			String sql = "SELECT m.*,t3.id as thirdId,t3.goodsThirdTypeName ,t3.vat,t3.consumptionTax,t3.consolidatedTax from "
					+ "(SELECT t1.id,t1.firstTypeName,t2.id as secId,t2.goodsSecondTypeName from ym_shop_goods_first_type t1 LEFT JOIN ym_shop_goods_second_type t2 ON (t1.id= t2.firstTypeId) WHERE t1.deleteFlag =0 AND t2.deleteFlag =0) m  "
					+ "LEFT JOIN ym_shop_goods_third_type t3 ON (m.secId = t3.secondTypeId) WHERE t3.deleteFlag =0";
			session = getSession();
