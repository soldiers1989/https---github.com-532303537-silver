...
public class StockDaoImpl extends BaseDaoImpl<Object> implements StockDao {
	...
	public Table getWarehousGoodsInfo(String merchantId, String warehouseCode, int page, int size, String entGoodsNo) {
		...
		try {
			String queryString = "SELECT m.* FROM (SELECT t1.customsCode,t2.goodsName,t2.entGoodsNo,t2.brand,t2.gUnit,t2.goodsStyle,t2.goodsDetailId,t2.regPrice FROM ym_shop_goods_record t1 LEFT JOIN ym_shop_goods_record_detail t2 ON t1.goodsSerialNo = t2.goodsSerialNo "
