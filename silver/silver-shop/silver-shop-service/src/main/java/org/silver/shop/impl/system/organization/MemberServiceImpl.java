...
import org.silver.shop.model.system.commerce.ShopCart;
...
public class MemberServiceImpl implements MemberService {
	...
	public Map<String, Object> addGoodsToShopCart(String memberId, String memberName, String goodsId, int count) {
		...
		if (reList == null) {
			...
			params.put("memberId", memberId);
			//查询当前用户购物车中是否有该商品
			List<Object> reShopCart = memberDao.findByProperty(ShopCart.class, params, 1, 1);
			if(reShopCart == null){
				statusMap.put(BaseCode.STATUS.getBaseCode(), StatusCode.WARN.getStatus());
				statusMap.put(BaseCode.MSG.getBaseCode(), StatusCode.WARN.getMsg());
				return statusMap;
			}else if(reShopCart.size()>0){
				ShopCart oldShopCart =	(ShopCart) reShopCart.get(0);
