...
public class PaymentDetailDao extends HibernateDaoImpl {

	public boolean add(PaymentDetail entity) {
	...
	public boolean delete(PaymentDetail entity) {
		Session session = null;
		try {
			session = getSession();
			Transaction tra = session.beginTransaction();
	...
	public PaymentDetail findMailboxbyId(long id) {

		Session session = null;
		try {
			session = getSession();
			PaymentDetail instance = (PaymentDetail) session.get(PaymentDetail.class, id);
			session.close();
			return instance;
		} catch (Exception re) {
			return null;
	...
	public Long findAllCount() {
		Session session = null;
		try {
			String hql = "select count(model) from PaymentDetail model ";
			session = getSession();
	...
	public List<PaymentDetail> findAll(int page, int size) {
		Session session = null;
		try {
			String hql = "from PaymentDetail model ";
			session = getSession();
			Query query = session.createQuery(hql);
			if (page > 0 && size > 0) {
	...
	public List findByProperty(String propertyName, Object value, int page, int size) {
		Session session = null;
		try {
			String queryString = "from PaymentDetail as model where model." + propertyName + "= ?";
			session = getSession();
	...
	public List<PaymentDetail> findByProperty(Map<String, Object> params, int page, int size) {
		Session session = null;
		try {
			session = getSession();
			String hql = "from PaymentDetail model ";
			...
			List<PaymentDetail> results = query.list();
			session.close();
			return results;
		} catch (Exception re) {
			re.printStackTrace();
	...
	public static void main(String[] args) {
		...
		PaymentDetailDao ed = new PaymentDetailDao();
