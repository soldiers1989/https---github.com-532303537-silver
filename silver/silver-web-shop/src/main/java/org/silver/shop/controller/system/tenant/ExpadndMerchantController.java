...
import org.silver.shop.service.system.tenant.ExpadndMerchantTransaction;
 ...
 * 拓展商controller层
 */
@RequestMapping("expadndMerchant")
@Controller
public class ExpadndMerchantController {

	@Autowired
	private ExpadndMerchantTransaction subMerchantTransaction;

	@RequestMapping(value = "/addInfo", method = RequestMethod.POST, produces = "application/json; charset=utf-8")
	@ApiOperation("添加拓展商户信息")
	...
	public String addInfo(HttpServletRequest req, HttpServletResponse response) {
		...
		if (!params.isEmpty()) {
			return JSONObject.fromObject(subMerchantTransaction.addInfo(params)).toString();
		}
